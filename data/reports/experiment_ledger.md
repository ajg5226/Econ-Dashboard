...
| C1 | Benchmark model family additions | DONE | DISCARD | 0.000 in-scope | 0.0000 | 0.0000 | Dot-com peak 0.230→0.230, GFC +0mo, S&L +0mo, COVID +0pp | Branch `experiment/C1-benchmark-models` SHA <stamped post-commit>. All 3 benchmarks (Hamilton, Chauvet-Piger, Wright probit) were **gated out of the active ensemble** by the `_select_active_models` CV gate: their pooled CV PR-AUCs (Hamilton 0.41, Chauvet-Piger 0.70, Wright 0.21) fall below 0.855 (=0.95 × 0.90 = probit's CV PR-AUC floor). Final ensemble = {probit, random_forest} at 0.5/0.5 weights — identical to baseline. Every in-scope delta is exactly 0. **Counterfactual peak check**: Hamilton raw series peaks at 0.84 during Dot-com, 1.00 during GFC/COVID; Chauvet-Piger peaks at 0.29 Dot-com and ~1.00 GFC/COVID — i.e., Hamilton COULD have lifted Dot-com if it cleared the gate, but pooled CV (1970+) dilutes its regime-specific strength against probit's across-the-board dominance. Equal-weight LOO Brier finds Hamilton would help (-1.4pp); Chauvet-Piger and Wright would hurt. **Finding**: external recession models don't add diversification vs the existing feature-based ensemble under the current DMA gate — the signal is either already subsumed (Chauvet-Piger correlates with probit yield-curve signal) or too weak in pooled CV (Hamilton's regime-specific value doesn't survive averaging across expansion folds). See `data/models/c1_validation.json` + `data/models/c1_variants/`. |
