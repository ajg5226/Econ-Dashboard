...
| B2 | Labor deterioration block upgrade | DONE | KEEP | +0.0223 | +0.0001 | ≈0 | GFC +1.0mo / COVID 0mo | Branch `experiment/B2-labor-block` SHA `97e7b9b` (parent `experiment/B1-at-risk-bakeoff`@`a4e0d89`). New series: JTSJOL, JTSQUR, CIVPART, EMRATIO, USGOOD, USSERV, UNEMPLOY. Winners in top-20: `GOODS_DECLINE_FLAG` (pos 15). Top marginal contributor: `GOODS_DECLINE_FLAG` (drop-ablation −5.08pp CV PR-AUC). hybrid (with B2) CV PR-AUC 0.9574 vs no_labor 0.9352 (+2.2pp); test PR-AUC 0.1937 vs 0.2525 (test set is 2015+ so JOLTS-era features have limited influence there — CV is the relevant metric). Two B2 features survive selection (`GOODS_DECLINE_FLAG`, `GOODS_YoY_MINUS_SERV_YoY`); JOLTS-dependent must-include items (`VU_RATIO_Z`, `QUITS_Z`) filtered by the 70%-non-null valid_cols gate because JOLTS starts Dec-2000 (~46% coverage in 1970+ frame). `GOODS_YoY_MINUS_SERV_YoY`'s marginal contribution is near-zero (ΔCV +0.004); the signal is carried almost entirely by `GOODS_DECLINE_FLAG`. Backtest: 5/6 for both variants (Dot-com misses; Oil Crisis throws pre-class error on both). Lead-time trade-offs: GFC 1.0→2.0mo, S&L 7.0→5.0mo, Volcker II 7.0→−6.0mo (regresses); Volcker I and COVID unchanged. `hybrid_no_b2_labor` was added as a 5th variant to `recession_engine/feature_variants.py`. See `data/models/b2_validation.json` + `data/models/b2_variants/`. |
