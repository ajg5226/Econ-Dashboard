...
| A1.75.2 | Re-test calibrators under in-scope-only safety gate | PLANNED | — | — | — | — | — | Scoped follow-on. Redefine safety probe to require Dot-com/GFC/COVID calibrated peaks ≥ 0.20 (in lieu of arbitrary mid-range probe points). Re-run A/B across all 7 A1.75 candidates. Likely to unlock the `post1990` calibrator (ECE 0.020) or `regime_stratified` (ECE 0.015) for deployment. Would reduce deployed ECE 0.122 → 0.02-0.04. Low-risk; all infrastructure exists on A1.75 branch. |
...
| B3 | Credit-supply block upgrade | PLANNED | — | — | — | — | — | Research top-5 #3 (with B2). Add SLOOS (DRTSCILM etc.), credit-growth × curve-inversion interactions, verify ANFCI. Focus: 1990/GFC lead times. First experiment under new scope policy. |
