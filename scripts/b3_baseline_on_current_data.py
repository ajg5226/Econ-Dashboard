"""
B3 helper: score pinned baseline_8df6b9d pickles on the with-B3 feature frame.

Adapted from ``scripts/a1_5_baseline_on_current_data.py`` but points at the
current (post-B2 + scope policy) baseline artifacts in
``data/models/baseline_8df6b9d/``. Writes
``data/models/b3_variants/baseline_8df6b9d_on_b3_data.json``.

This isolates the code delta (B3 features added) from retraining variance:
the baseline pickles score the new feature frame by zero-padding columns
that didn't exist at baseline training time.
"""
...
logger = logging.getLogger("b3_baseline_on_current_data")
...
def main() -> None:
    baseline_dir = ROOT / "data" / "models" / "baseline_8df6b9d"
    ...
    logger.info("Fetching + engineering features (current B3 data)...")
    ...
    results = {
        ...
        "baseline_git_sha": "8df6b9d",
        "scored_on_feature_frame": "post-B3 (B3 features in df, but baseline only uses its original 50)",
        ...
        "notes": (
            "Pinned baseline_8df6b9d pickles scored on the post-B3 feature "
            "frame's last 134 labeled rows. Isolates the code delta (B3 "
            "features added) from retraining variance: the baseline doesn't "
            "actually use B3 features, so this should be nearly identical to "
            "the at-freeze metrics unless the shared feature space subtly "
            "changed."
        ),
    }
    if p_xgb is not None:
    ...
    out_path = ROOT / "data" / "models" / "b3_variants" / "baseline_8df6b9d_on_b3_data.json"
    out_path.parent.mkdir(parents=True, exist_ok=True)
    out_path.write_text(json.dumps(results, indent=2, default=str))
    logger.info("Wrote %s", out_path)
    logger.info(
        "Baseline ensemble on B3 data: AUC=%s PR-AUC=%s Brier=%s",
